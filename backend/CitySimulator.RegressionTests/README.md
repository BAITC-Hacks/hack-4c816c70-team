...
This dependency-free executable returns a nonzero exit code on failure. It covers exact decimal rounding, baseline/control results, clamp bounds, per-measure effects, validated claim ordering and malformed LLM response shapes. HTTP is an in-memory double: no OpenAI account, real API key, model access or paid requests are required.

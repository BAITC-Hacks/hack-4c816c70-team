...
| LLM_MODE=live после исправления рекомендаций (`a30ea47`) | Эталон в браузере: `52,56 → 56,54`, 95/5, `explanationSource: "llm"`, подпись «Текст подготовлен серверным AI». Рекомендации формирует сервер: «Заменить M5 (Сарыарка) на M3 (Нура)…», варианты применяются по отдельности. Модель один раз ошибочно назвала самым слабым районом Сарыарку (см. отчёт, дефект 6) |

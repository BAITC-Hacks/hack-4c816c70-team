...
В POST `/api/simulations/evaluate` передавайте `Accept-Language: ru-RU`, `kk-KZ` или `en-US`. Русский используется по умолчанию. Поддерживаются короткие теги и приоритеты `q`; неподдерживаемые языки пропускаются.

Ответ содержит `explanationLocale` с фактической локалью и заголовок `Content-Language`. Все четыре поля объяснения, включая рекомендации, локализованы и в mock, и при fallback. Для русского и казахского текст использует десятичную запятую, для английского — точку. Числовые поля JSON и тело запроса сохраняются: эталон во всех языках даёт расходы 95 и Score 56.54.

GPT подключён только в C# backend через встроенный `HttpClient` к OpenAI Responses API (`POST /v1/responses`). Модель берётся из `OPENAI_MODEL` (проверена `gpt-4o-mini`), ключ — из `OPENAI_API_KEY` в локальном `.env`. Модель возвращает порядок ID проверенных утверждений; переводы, факты и числа формирует сервер. Ключ и вызов OpenAI не передаются в браузер.

Проверка трёх языков с локальной заглушкой OpenAI: `python3 backend/scripts/verify-explanation-locales.py` из корня проекта в WSL. Реальный ключ не нужен.

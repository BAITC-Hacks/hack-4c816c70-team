...
В текущем коде реализованы API сценария и оценки, серверная валидация, расчёт Score, объяснение в режимах mock/live, Swagger и контейнерный запуск. Frontend содержит шаги «Обзор», «Решения» и «Результат», загрузку сценария и отправку выбора в API. Полный браузерный сценарий нового интерфейса ещё нужно принять по разделу 8; наличие экранов не означает, что эта проверка уже выполнена.
...
Вход в систему не требуется. Ниже ожидаемый сценарий приёмки нового интерфейса; его прохождение в браузере нужно проверить отдельно.

...
| Браузер, WSL Ubuntu, 8080/3000 | Контрольный набор в интерфейсе: `52,56 → 56,54`, 95/5, районы, синергия, объяснение; ограничения выбора и ошибки API отображаются. На 390 px на шаге «Решения» есть горизонтальная прокрутка 4 px |
| LLM_MODE=live после исправления рекомендаций | Эталонный запрос: Score 56.54, explanationSource=llm; OpenAI 200, 3,9 с. Рекомендации собирает сервер с исходным и целевым районами; повторная браузерная проверка ожидается |
| Публичный API из браузера со страницы `localhost:3000` | `GET /api/scenario` → `200`, CORS разрешён |

Контейнерная проверка выполнена в WSL Arch Linux на отдельных тестовых портах 18080 и 13000. Значения по умолчанию для команд ниже — 8080 и 3000. Браузерная проверка выполнена в WSL Ubuntu-24.04 на портах 8080 и 3000, запуск из чистой копии в mock — там же на 8081 и 3001 ([отчёт](docs/reports/e2e.md)).
...
В модель передаются только рассчитанные сервером факты: Score, показатели районов, выбранные меры и синергии. Запрос использует store: false и строгую JSON-схему из трёх полей: summary, strengths, risks. Сервер проверяет структуру, длину текста, числа и идентификаторы выбранных мер; при отказе модели или невалидном ответе возвращает шаблон. Рекомендации всегда формирует RecommendationBuilder из проверенных замен, с явными исходным и целевым районами. Варианты замен в модель не отправляются. Внешний ответ explanation сохраняет четыре поля, включая recommendations.

Общий предел AI-анализа — **60 секунд**, включая все попытки и паузы. Сетевые ошибки и HTTP 408/409/429/5xx повторяются не более двух раз внутри этого предела; затем API возвращает `mock`. Для frontend достаточно таймаута запроса 75 секунд. Поле `explanationSource` равно `llm` при принятом ответе модели или `mock` при шаблонном объяснении. В логах — номер попытки, HTTP-статус, время и число токенов; при отклонении чисел также их список. Ключ, тело запроса и полный текст модели не записываются.
...
Вход в систему не требуется. Ниже сценарий приёмки интерфейса; фактический прогон — в [`docs/reports/e2e.md`](docs/reports/e2e.md).

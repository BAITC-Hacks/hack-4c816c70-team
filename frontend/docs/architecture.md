...
Статус: архитектурный план и задания для реализации. Первичное обследование выполнено на `325da89`; перед передачей получен commit команды `8117017`, реализующий C# scenario/evaluate и Docker. Фронтенд пока содержит стартовую страницу. Live Swagger и сквозной сценарий из этой задачи не проверялись; эти документы не означают, что приложение уже готово.
...
7. Ни симуляции, ни AI-вызовов в Next.js. Next может перенаправлять `/api/*` в C# без изменения payload и статусов.
8. За зависимости и общий монтаж отвечает только GPT. Два Claude строят независимые модули по общему UI-контракту.

Документация Next.js подтверждает используемые границы [Server и Client Components](https://nextjs.org/docs/app/getting-started/server-and-client-components), [структуру App Router](https://nextjs.org/docs/app/getting-started/project-structure) и [перенаправление запросов через rewrites](https://nextjs.org/docs/app/api-reference/config/next-config-js/rewrites). Выбор модулей и ролей — решение этой команды.
...
Score, районные баллы, эффекты с лагом, применённые синергии и любые прогнозные показатели приходят с C#. Не вычисляйте их по датасету. Все показатели направлены одинаково: больше — лучше. Отрицательное изменение T1 у M11 должно быть заметно.

Значения API сохраняются с исходной точностью. Форматирование `Intl.NumberFormat('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })` применяется при выводе. Не округляйте данные в адаптере. Для дельт используйте поля API; если их нет, выводите «до → после» до согласования. Нет поля районного балла в scenario — показываем показатели без выдуманного балла.
...
Браузер обращается к относительным `/api/scenario` и `/api/simulations/evaluate`. В локальном dev GPT настраивает rewrite `/api/:path*` на C# через server-only `API_INTERNAL_URL`, например `http://localhost:8080`.

В compose из `8117017` C# сервис называется `api`, его внутренний адрес — `http://api:8080`. Он не должен попасть в браузер. Текущая конфигурация команды передаёт клиентский `NEXT_PUBLIC_API_URL=http://localhost:8080`; это отличается от целевого same-origin подхода. GPT согласует переход с владельцем сервера и не ломает работающий compose. До перехода допустим существующий публичный base URL с настроенным CORS. Rewrites из production-сборки нужно проверять с итоговым окружением сборки: нельзя рассчитывать, что смена env после build автоматически перепишет destination. Участник сервера передаёт нужный адрес на этапе сборки либо предоставляет внешний reverse proxy для `/api/*`.

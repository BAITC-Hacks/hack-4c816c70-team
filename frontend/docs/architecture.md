...
Решение пользователя от 23 сентября 2026: Intel AI Playground Studio и локальные модели на его Intel Arc исключены из плана из-за нехватки оперативной памяти. Интеграцию с Playground, запуск локального llama.cpp/OpenVINO и зависимость демо от GPU не реализуем. Базовый запуск сохраняет серверный `LLM_MODE=mock`; подключение внешнего live LLM при необходимости выполняет владелец C# API, без изменения фронтенд-контракта.
...
Коммит и push промежуточной работы — не реже 30 минут по регламенту команды. До 15:00 приоритет — путь выбора и расчёт без LLM. По уточнению пользователя целевой срок готовности — 17:30, 18:00 остаётся резервом. Новые функции замораживаем в 16:30; до 17:15 выполняем чистый compose-запуск, проверки и исправления, к 17:30 — финальный push и готовая демонстрация. Если контрольная точка уже прошла, сначала восстанавливаем рабочий сквозной путь, затем добавляем визуальные улучшения.

...
export const resultMessages: Record<Locale, {
  ...
  readonly attempts: {
    readonly currentMeasures: string;
    ...
    readonly districtScores: string;
    readonly measures: string;
    readonly wholeCity: string;
    readonly onlyHere: string;
    readonly scoreUp: (delta: string) => string;
    readonly scoreDown: (delta: string) => string;
    readonly scoreSame: string;
  };
}> = {
  ru: {
    eyebrow: "Результат симуляции", heading: "Ваш план оценён", submitted: (selected, required, quarters) => `${selected} из ${required} решений отправлены на сервер и рассчитаны для горизонта ${quarters} кварталов.`, edit: "Изменить решения", reset: "Начать заново", finalBudget: "Финальный бюджет", budgetAria: (spent, budget) => `Потрачено ${spent} из ${budget}`, remaining: (remaining) => `Остаток: ${remaining} ед.`, scoreAria: "Сравнение итогового Score", finalScore: "Итоговый Score", before: "До", after: "После", scoreBarAria: (before, after) => `Score: ${before} до, ${after} после`, change: "Изменение", districts: "Районы: до и после", indicators: "Показатели: до и после", below: (threshold) => `Остаётся ниже ${threshold}`, recovered: (threshold) => `Выведен из критической зоны (было ниже ${threshold})`, synergies: "Применённые синергии", explanation: "Объяснение от сервера", strengths: "Сильные стороны", risks: "Риски", recommendations: "Рекомендации", none: "Нет.", staleExplanation: (language) => `Объяснение получено на языке «${language}». Числа от языка не зависят. Чтобы получить объяснение на русском, повторите оценку с теми же решениями.`, reevaluate: "Повторить оценку на русском", reevaluating: "Повторная оценка…", source: { llm: "AI-анализ", mock: "Шаблонное объяснение" }, attempts: { currentMeasures: "Выбранные меры", comparisonTitle: "Сравнение двух последних попыток", comparisonNote: "Две последние успешные оценки с разными наборами решений. Числа взяты из ответов сервера, разница — вычитание показанных значений.", metric: "Показатель", previous: "Предыдущая", current: "Текущая", difference: "Разница", score: "Score", spent: "Расходы", remaining: "Остаток", districtScores: "Оценки районов", measures: "Меры и районы", wholeCity: "Весь город", onlyHere: "нет в другой попытке", scoreUp: (delta) => `Score вырос на ${delta} по сравнению с предыдущей попыткой.`, scoreDown: (delta) => `Score снизился на ${delta} по сравнению с предыдущей попыткой.`, scoreSame: "Score не изменился по сравнению с предыдущей попыткой." },
  },
  kk: {
    eyebrow: "Симуляция нәтижесі", heading: "Жоспарыңыз бағаланды", submitted: (selected, required, quarters) => `${selected} / ${required} шешім серверге жіберіліп, ${quarters} тоқсан көкжиегіне есептелді.`, edit: "Шешімдерді өзгерту", reset: "Қайта бастау", finalBudget: "Қорытынды бюджет", budgetAria: (spent, budget) => `${spent} / ${budget} жұмсалды`, remaining: (remaining) => `Қалдық: ${remaining} бірл.`, scoreAria: "Қорытынды Score салыстыруы", finalScore: "Қорытынды Score", before: "Бұрын", after: "Кейін", scoreBarAria: (before, after) => `Score: бұрын ${before}, кейін ${after}`, change: "Өзгеріс", districts: "Аудандар: бұрын және кейін", indicators: "Көрсеткіштер: бұрын және кейін", below: (threshold) => `${threshold} деңгейінен төмен болып қалды`, recovered: (threshold) => `Сындарлы аймақтан шықты (бұрын ${threshold} деңгейінен төмен)`, synergies: "Қолданылған синергиялар", explanation: "Сервер түсіндірмесі", strengths: "Күшті жақтары", risks: "Тәуекелдер", recommendations: "Ұсынымдар", none: "Жоқ.", staleExplanation: (language) => `Түсіндірме «${language}» тілінде алынған. Сандар тілге тәуелді емес. Түсіндірмені қазақ тілінде алу үшін сол шешімдермен қайта бағалаңыз.`, reevaluate: "Қазақ тілінде қайта бағалау", reevaluating: "Қайта бағалануда…", source: { llm: "AI-талдау", mock: "Үлгілік түсіндірме" }, attempts: { currentMeasures: "Таңдалған шаралар", comparisonTitle: "Соңғы екі әрекетті салыстыру", comparisonNote: "Шешімдер жиынтығы әртүрлі соңғы екі сәтті бағалау. Сандар сервер жауаптарынан алынды, айырма — көрсетілген мәндердің айырымы.", metric: "Көрсеткіш", previous: "Алдыңғы", current: "Ағымдағы", difference: "Айырма", score: "Score", spent: "Шығындар", remaining: "Қалдық", districtScores: "Аудандардың ұпайлары", measures: "Шаралар мен аудандар", wholeCity: "Бүкіл қала", onlyHere: "басқа әрекетте жоқ", scoreUp: (delta) => `Score алдыңғы әрекетпен салыстырғанда ${delta} өсті.`, scoreDown: (delta) => `Score алдыңғы әрекетпен салыстырғанда ${delta} төмендеді.`, scoreSame: "Score алдыңғы әрекетпен салыстырғанда өзгерген жоқ." },
  },
  en: {
    eyebrow: "Simulation result", heading: "Your plan has been evaluated", submitted: (selected, required, quarters) => `${selected} of ${required} decisions were sent to the server and calculated for a ${quarters}-quarter horizon.`, edit: "Edit decisions", reset: "Start over", finalBudget: "Final budget", budgetAria: (spent, budget) => `${spent} of ${budget} spent`, remaining: (remaining) => `Remaining: ${remaining} units`, scoreAria: "Final Score comparison", finalScore: "Final Score", before: "Before", after: "After", scoreBarAria: (before, after) => `Score: ${before} before, ${after} after`, change: "Change", districts: "Districts: before and after", indicators: "Indicators: before and after", below: (threshold) => `Remains below ${threshold}`, recovered: (threshold) => `Out of the critical zone (was below ${threshold})`, synergies: "Applied synergies", explanation: "Server explanation", strengths: "Strengths", risks: "Risks", recommendations: "Recommendations", none: "None.", staleExplanation: (language) => `This explanation was received in ${language}. Numbers do not depend on language. Re-run the evaluation with the same decisions to get it in English.`, reevaluate: "Re-evaluate in English", reevaluating: "Re-evaluating…", source: { llm: "AI analysis", mock: "Template explanation" }, attempts: { currentMeasures: "Selected measures", comparisonTitle: "Comparison of the last two attempts", comparisonNote: "The last two successful evaluations with different decision sets. Numbers come from server responses; the difference subtracts the displayed values.", metric: "Metric", previous: "Previous", current: "Current", difference: "Difference", score: "Score", spent: "Spending", remaining: "Remaining", districtScores: "District scores", measures: "Measures and districts", wholeCity: "Whole city", onlyHere: "not in the other attempt", scoreUp: (delta) => `Score rose by ${delta} compared with the previous attempt.`, scoreDown: (delta) => `Score fell by ${delta} compared with the previous attempt.`, scoreSame: "Score did not change compared with the previous attempt." },
  },
};

...
export function Simulator() {
  ...
  return <div className={styles.shell}>
    ...
    {scenario ? <main className={styles.main}>
      {state.step === "overview" ? <CityOverview scenario={scenario} selectedDistrictId={state.selectedDistrictId} onSelectDistrict={(districtId) => dispatch({ type: "select-district", districtId })} onStartPlanning={() => dispatch({ type: "set-step", step: "planner" })} /> : null}
      {state.step === "planner" ? <Planner scenario={scenario} choices={state.choices} preferredDistrictId={state.selectedDistrictId} isEvaluating={state.evaluation.status === "pending"} serverError={state.evaluation.status === "error" ? state.evaluation.message : null} onChoicesChange={setChoices} onEvaluate={() => void evaluate()} onBack={() => dispatch({ type: "set-step", step: "overview" })} /> : null}
      {state.step === "results" && state.evaluation.status === "success" ? <Results scenario={scenario} result={state.evaluation.result} submittedChoices={state.evaluation.submittedChoices} onEdit={() => dispatch({ type: "set-step", step: "planner" })} onReset={reset} /> : null}
    </main> : null}
  </div>;
}
